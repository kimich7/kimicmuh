...
if (isset($_POST["action"])&&($_POST["action"]=="Edit")) {
    ...
    $title=$_POST["title"];//工程名稱
    $process=$_POST["process"];//工程流程進度及金額
    $content=$_POST["content"];//工程進度/異常說明
    $contract=$_POST["contract"];//日期、決包廠商、金額：
    $building=$_POST["building"];//工程範圍(位置)
    ...
    $stmtM->bindParam(':content',$content,PDO::PARAM_STR);
    $stmtM->bindParam(':contract',$contract,PDO::PARAM_STR);
    $stmtM->bindParam(':building',$building,PDO::PARAM_STR);
    $stmtM->bindParam(':id',$id,PDO::PARAM_STR);
    $stmtM->execute();
    if ($compare!=$content) {
        $content=str_replace(chr(13).chr(10), "<br/>",$content);
        $minsertstr="INSERT INTO FA.process(id,pmc_id,related_id,content,createdOn,createdUser,status)VALUES('$pid','$id','','$content','$datetime1','$userID','Y')";
        $minsert=$pdo->exec($minsertstr);
    }
    $pdo=null;
                ...
                <div class="panel-body">
                    <div class="row">
                        <div class="col-lg-7">
                            ...
                            <?php if ($editor==1) {?>
                                <textarea class="content_box" rows="5" name="title"><?php echo $title; ?></textarea>
                            <?php } else {?>
                                <span title="非負責人員無法修改">
                                <textarea class="content_box" rows="5" name="title" Disabled><?php echo $title; ?></textarea>
                                </span>
                            <?php }?>                       
                            </div>
                            <!--工程流程進度及金額process-->
                            ...
                            <?php if ($editor==1) {?>
                                <textarea class="content_box" rows="5" name="process"><?php echo $process; ?></textarea>
                            <?php } else {?>
                                <span title="非負責人員無法修改">
                                <textarea class="content_box" rows="5" name="process" Disabled><?php echo $process; ?></textarea>
                                </span>
                            <?php }?> 
                            </div>
                            <!--工程進度/異常說明content-->
                            ...
                            <?php if ($editor==1) {?>
                                <textarea class="content_box" rows="6" name="content"><?php echo $content; ?></textarea>
                            <?php } else {?>
                                <span title="非負責人員無法修改">
                                <textarea class="content_box" rows="6" name="content" Disabled><?php echo $content; ?></textarea>
                                </span>
                            <?php }?>
                            </div>
                            <!--日期、決包廠商、金額contract-->
                            ...
                            <?php if ($editor==1) {?>
                                <textarea class="content_box" rows="4" name="contract"><?php echo $contract; ?></textarea>
                            <?php } else {?>
                                <span title="非負責人員無法修改">
                                <textarea class="content_box" rows="4" name="contract" Disabled><?php echo $contract; ?></textarea>
                                </span>
                            <?php }?>
                            </div>
                            <!--工程範圍(棟別)building-->
                            ...
                            <?php if ($editor==1) {?>
                                <input type="text" class="content_box" rows="4" name="building" value="<?= $building ?>">
                            <?php } else {?>
                                <span title="非負責人員無法修改">
                                <input type="text" class="content_box" rows="4" name="building" value="<?= $building ?>" Disabled>
                                </span>
                            <?php }?>
                            </div>
                            <!--承辦人-->                            
                            ...
                            <div class="form-group"> 
                                <?php if ($editor==1) {?>
                                    <button type="submit"
                                        class="btn btn-primary mt-4" formtarget="_self">送出</button>
                                <?php } else {?>
                                    <span data-toggle="tooltip" data-placement="bottom"title="非負責人員無法修改">
                                    <button style="pointer-events: none;" type="submit"
                                        class="btn btn-primary mt-4" formtarget="_self" disabled>送出</button>
                                    </span>
                                <?php }?>                               
                            </div>
                        </div>
