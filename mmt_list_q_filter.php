...
<!-- header網頁標題 -->
    ...
    <h1 class="text-center">水系統-給水(設備點檢)</h1>
        <div class="d-flex ">        
            <a href='mmt_list_q.php' type="button" id="backlistmmt" class="btn btn-primary mt-4 rounded d-block mr-3">清除條件</a>
        </div> 
        ...
        <?php
            ...
            switch ($x) {
                case 1://只有keyword
                    ...
                    //總資料數量
                    $totalstr_num="SELECT COUNT(DISTINCT(a.id)) FROM FA.MMT_QtableM as a LEFT JOIN FA.Employee as b on a.emp=b.e_number or a.sremp=b.e_number or a.cemp=b.e_number left join FA.MMT_build as c on a.bid=c.id left join FA.MMT_equip as s on a.eid=s.id left join FA.MMT_KIND as k on a.tid=k.id LEFT JOIN FA.CheckStatus as p ON a.status=p.id WHERE b.cname like '%$keyword%' or c.bName like '%$keyword%' or s.eName like '%$keyword%' or k.tableName like '%$keyword%' or p.checkName like '%$keyword%' or a.remark like '%$keyword%' or a.rdate like '%$keyword%' ";
                    break;
                case 2://只有日期區間
                    //所有的資料 (check_manager IS NULL or check_manager=0) and
                    $sqlstr_total=" SELECT * FROM FA.MMT_QtableM WHERE rdate between '$start_date' and '$end_date'";
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT * FROM FA.MMT_QtableM WHERE rdate between '$start_date' and '$end_date' ORDER BY id DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                    $totalstr_num="SELECT COUNT(id) FROM FA.MMT_QtableM WHERE rdate between '$start_date' and '$end_date' ";
                    break;
                case 3://只有開始時間
                    //所有的資料 (check_manager IS NULL or check_manager=0) and
                    $sqlstr_total=" SELECT * FROM FA.MMT_QtableM WHERE rdate ='$start_date'";
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT * FROM FA.MMT_QtableM WHERE rdate ='$start_date' ORDER BY id DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                    $totalstr_num="SELECT COUNT(id) FROM FA.MMT_QtableM WHERE rdate ='$start_date'";
                    break;
                case 4://只有結束時間
                    //所有的資料 (check_manager IS NULL or check_manager=0) and
                    $sqlstr_total=" SELECT * FROM FA.MMT_QtableM WHERE rdate ='$end_date'";
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT * FROM FA.MMT_QtableM WHERE rdate ='$end_date' ORDER BY id DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                    $totalstr_num="SELECT COUNT(id) FROM FA.MMT_QtableM WHERE rdate ='$end_date'";
                    break;
                case 5://開始+keyword
                    //所有的資料 (check_manager IS NULL or check_manager=0) and
        ...
        ?>
        <table id="mmt_q" class="display table table-striped table-bordered table-hover col-xl-2 col-lg-2 col-md-4 col-sm-12 col-12 table-sm order-table"  aria-describedby="dataTables-example_info" data-sort-name="tid" data-sort-order="desc" data-sortable ="true"><!--表格樣式：條紋行、帶框表格、可滑入行-->
        ...
        ?>
            <tr align="center">
                ...
                <td><a href="mmtDetail_q.php?id=<?= $ammtm[$i]['id']?>"><?= $ammtmtabletitle.'('.$macNo.')' ?></a></td><!--表單名稱-->
                ...
                if ($ammtmstatus!='審核完成') { ?>                
                    <td>
                        ...
                        <a href="mmtcheck_q.php?id=<?= $ammtm[$i]['id']?>" class="waterL2 waterL3" Disabled><?= $ammtmstatus?></a>
                        </span>
                    </td><!--狀態-->
                ...
                <td>
                    <span class="billBoardwaterL2 billBoardwaterL3" tabindex="0" data-toggle="tooltip" data-placement="bottom" title="請登入相應權限帳號已解鎖">
                    <a href="mmtEdit_q.php?id=<?= $ammtm[$i]['id']?>" class="waterL2 waterL3" Disabled>內容編輯與修改</a>
                    </span>
                </td>                
            </tr>
        <?php }?>
        ...
        <?php
            ...
            echo '<table border="0" align="center">';
                echo '<tr>';
                    if ($page_num>1) {
                        @$prev=$page_num-1;
                        echo "<td><a href='mmt_list_q_filter.php?page=1\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>"."[第一頁]".'</a></td>';
                        echo "<td><a href='mmt_list_q_filter.php?page={$prev}\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>"."[<<<上一頁]".'</a></td>';
                        
                    }
                    if ($page_num<$total_page) {
                        @$next=$page_num+1;
                        echo "<td>"."<a href='mmt_list_q_filter.php?page={$next}\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>"."[下一頁>>>]".'</a></td>';
                        echo "<td><a href='mmt_list_q_filter.php?page=$total_page\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>".'[最末頁]'.'</a></td>';
                    }
                echo '</tr>';
            echo '</table>';
            
            //分頁按鈕一次七頁
            $phpfile = 'mmt_list_q_filter.php';
    ...
    <form action="mmt_list_q_filter.php" method="post" name="mtlist"> 
            <div>
                <div align="left">
                    日期區間搜索：</br>
