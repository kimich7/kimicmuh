...
//20190703新增-判斷權限分類
    ...
    $securityNoStr_emp="SELECT e.sid,e.e_number FROM FA.securityemp as e LEFT JOIN FA.securityKind as k on e.sid=k.id  WHERE e.e_number='$userID' and k.id = 22";
    $securityNo_emp=$pdo->query($securityNoStr_emp)->fetch();
    $securityNoStr_PM="SELECT e.sid,e.e_number FROM FA.securityemp as e LEFT JOIN FA.securityKind as k on e.sid=k.id  WHERE e.e_number='$userID' and k.id = 23";
    $securityNo_PM=$pdo->query($securityNoStr_PM)->fetch();
    $checksum=0;
    if (!empty($securityNo_emp) and $securityNo_emp!='') {
        $checksum=$checksum+2;//可簽核-專責        
    }
    if(!empty($securityNo_PM) and $securityNo_PM!=''){
        $checksum=$checksum+1;//可簽核-主管
    }
...
if (isset($_POST["action"])&&($_POST["action"]=="check")) {    
    ...
    if (empty($_POST["mmt_g_Check"])) {
        switch ($checksum) {
            case 1:
                $cemp=null;
                $MasterStr="UPDATE FA.MMT_GtableM SET status='M',cemp =:cemp WHERE id=:mid";
            ...
            case 2:
                $cemp=null;
                $MasterStr="UPDATE FA.MMT_GtableM SET status='W',sremp =:cemp WHERE id=:mid";
                $stmtM = $pdo->prepare($MasterStr);
                $stmtM->bindParam(':mid',$mid,PDO::PARAM_STR);
                $stmtM->bindParam(':cemp',$cemp,PDO::PARAM_STR);
                $stmtM->execute();    
            ...
            case 3:   
                $cemp=null;             
                $MasterStr="UPDATE FA.MMT_GtableM SET status='W',sremp =:sremp,cemp =:cemp WHERE id=:mid";
                $stmtM = $pdo->prepare($MasterStr);
                $stmtM->bindParam(':mid',$mid,PDO::PARAM_STR);
                $stmtM->bindParam(':cemp',$cemp,PDO::PARAM_STR);
        ...
        switch ($checksum) {
            case 1:                
                @$check_ans=$_POST["mmt_g_Check"];//審核
            ...
            case 2:                
                @$check_ans=$_POST["mmt_g_Check"];//審核
                $cemp=$_POST["memp"];//審核者(當下登入的)                
                $MasterStr="UPDATE FA.MMT_GtableM SET status=:status,sremp=:cemp WHERE id=:mid";
                $stmtM = $pdo->prepare($MasterStr);
                $stmtM->bindParam(':status',$check_ans,PDO::PARAM_STR);
                $stmtM->bindParam(':cemp',$cemp,PDO::PARAM_STR);
                $stmtM->bindParam(':mid',$mid,PDO::PARAM_STR);
                ...
                break;
            case 3:
                $check_ansArray=$_POST["mmt_g_Check"];//審核
                $cemp=$_POST["memp"];//審核者(當下登入的)                
                ...
                if ($check_ans=='F,M' or $check_ans=='F' or $check_ans=='M,F') {
                    $MasterStr="UPDATE FA.MMT_GtableM SET status='F',sremp=:sremp,cemp=:cemp WHERE id=:mid";
                    $stmtM = $pdo->prepare($MasterStr);
                    // $stmtM->bindParam(':status',$check_ans,PDO::PARAM_STR);
                    $stmtM->bindParam(':sremp',$cemp,PDO::PARAM_STR);
                    $stmtM->bindParam(':cemp',$cemp,PDO::PARAM_STR);                
        ...
        if ($checksum==1) {   //主管登錄 ?>
            <div class="row my-3">
                ...
                <?php if ($Mdata[0]['status']=='W'or $Mdata[0]['status']=='' or $Mdata[0]['status']==null) {?>
                    <p class="d-inline font-weight-bold" name="reMumber">工務室：<?= $username?>&nbsp&nbsp&nbsp<input type="checkbox" name="mmt_g_Check" value='F' disabled>審核</p>
                <?PHP } ?>
                <?php if ($Mdata[0]['status']=='M') {?>
                    <p class="d-inline font-weight-bold" name="reMumber">工務室：<?= $username?>&nbsp&nbsp&nbsp<input type="checkbox" name="mmt_g_Check" value='F'>審核</p>
                <?PHP } ?>
                <?php if ($Mdata[0]['status']=='F') {?>
                    <p class="d-inline font-weight-bold" name="reMumber">工務室：<?= $username?>&nbsp&nbsp&nbsp<input type="checkbox" name="mmt_g_Check" value='F' checked>審核</p>
                <?PHP } ?>
                </div>
            ...
            <div class="row my-3">
                ...
                <?php 
                    if ($Mdata[0]['status']=='W'or $Mdata[0]['status']=='' or $Mdata[0]['status']==null) {?>
                        <p class="d-inline font-weight-bold">專責人員：<?= $username?>&nbsp&nbsp&nbsp<input type="checkbox" name="mmt_g_Check" value='M' >審核</p>
              <?PHP } 
                    if($Mdata[0]['status']=='M') { ?>
                        <p class="d-inline font-weight-bold">專責人員：<?= $username?>&nbsp&nbsp&nbsp<input type="checkbox" name="mmt_g_Check" value='M' checked>審核</p>
              <?PHP }
                    if($Mdata[0]['status']=='F') { ?>
                        <p class="d-inline font-weight-bold">專責人員：<?= $username?>&nbsp&nbsp&nbsp<input type="checkbox" name="mmt_g_Check" value='M' checked Disabled>審核</p>
              <?PHP }?>
                </div>
                <div class="col text-right">
                    <p class="d-inline font-weight-bold">保養人員：<?= $remp ?></p>
