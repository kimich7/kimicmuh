...
//總資料
//$ammtmstr="SELECT * FROM FA.MMT_QtableM ";
...
//所有的資料
$ammtmstr="SELECT id,bid,fid,eid,rdate,datekind,tid,macNo,remark,emp,sremp,cemp,status FROM FA.MMT_QtableM ";//全部資料
//總資料數量
$ammtmnumstr="SELECT Count(id)FROM FA.MMT_QtableM ";
$ammtmnum=Current($pdo->query($ammtmnumstr)->fetch());//全部數量
$total_num=$ammtmnum;

//----還沒帶入----
//篩選後給每頁的筆數
$sqlstr_page="SELECT id,bid,fid,eid,rdate,datekind,tid,macNo,remark,emp,sremp,cemp,status FROM FA.MMT_QtableM  ORDER BY rdate DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
...
<body>
    ...
    <form action="mmtCreate_q_choice.php" method="post" name="mmtca">
        <div class="panel-heading">
            <input type="hidden" name="mmtsysa" value='Q'>
            <div class="row my-3">                
                <div class="col">                    
 ...
 //計算總頁數
        ...
        echo '<table border="0" align="center">';
            echo '<tr>';
                if ($page_num>1) {
                    $prev=$page_num-1;
                    echo '<td><a href="mmt_list_q.php?page=1">'."[第一頁]".'</a></td>';
                    echo "<td><a href=\"mmt_list_q.php?page={$prev}\">"."[<<<上一頁]".'</a></td>';
                }
                if ($page_num<$total_page) {
                    $next=$page_num+1;
                    echo "<td>"."<a href=\"mmt_list_q.php?page={$next}\">"."[下一頁>>>]".'</a></td>';
                    echo "<td><a href=\"mmt_list_q.php?page=$total_page\">".'[最末頁]'.'</a></td>';
                }
            echo '</tr>';
        echo '</table>';
        //分頁按鈕一次七頁
            $phpfile = 'mmt_list_q.php';
