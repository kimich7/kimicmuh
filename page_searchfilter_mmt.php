...
function page($page,$total,$phpfile,$startDate,$endDate,$keyword,$sdid,$hidden,$pagesize=10,$pagelen=7){
    ...
    if($page!=1){
        $pagecode.="<a href={$phpfile}?page=1\"&startdate=\"".$startDate."\"&enddate=\"".$endDate."\"&keyword=\"".$keyword."\"&key=\"".$hidden."\"&sdid=\"".$sdid."\">&lt;&lt;</a>";//第一頁
        $pagecode.="<a href={$phpfile}?page=".($page-1)."\"&startdate=\"".$startDate."\"&enddate=\"".$endDate."\"&keyword=\"".$keyword."\"&key=\"".$hidden."\"&sdid=\"".$sdid."\">&lt;</a>";//上一頁
    }
    //分頁數大於頁碼個數時可以偏移
    if($pages>$pagelen){
        //如果當前頁小於等於左偏移
    ...
    for($i=$init;$i<=$max;$i++){
        if($i==$page){
            ...
            $pagecode.="<a href={$phpfile}?page={$i}&startdate=\"".$startDate."\"&enddate=\"".$endDate."\"&keyword=\"".$keyword."\"&key=\"".$hidden."\"&sdid=\"".$sdid."\">$i</a>";
        }
    }
    if($page!=$pages){
        $pagecode.="<a href={$phpfile}?page=".($page+1)."\"&startdate=\"".$startDate."\"&enddate=\"".$endDate."\"&keyword=\"".$keyword."\"&key=\"".$hidden."\"&sdid=\"".$sdid."\">&gt;</a>";//下一頁
        $pagecode.="<a href={$phpfile}?page={$pages}\"&startdate=\"".$startDate."\"&enddate=\"".$endDate."\"&keyword=\"".$keyword."\"&key=\"".$hidden."\"&sdid=\"".$sdid."\">&gt;&gt;</a>";//最後一頁
    }
    $pagecode.='</div>';
