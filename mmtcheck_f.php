...
if (isset($_POST["action"])&&($_POST["action"]=="Edit")) {
    ...
    $S1check=$_POST['S1Check'];//第一季確認結果
    $S2check=$_POST['S2Check'];//第二季確認結果
    $S3check=$_POST['S3Check'];//第三季確認結果
    $S4check=$_POST['S4Check'];//第四季確認結果
    $checknum=0;
    echo $_POST['S1Check'];
    if ($_POST['S1Check']=='done') {
        $S1man=$_POST['S1manager'];//第一季審核者
        $checknum=$checknum+1;
        for ($i=0; $i < $num; $i++) { 
        $S1_id=$i+800;//S1 id欄位的名稱
        $dids1=$_POST["$S1_id"];//取得S1的ID
        $sql="UPDATE FA.MMT_FtableD SET checkmanid=:checkmanid WHERE id=:ID";
        $stmt = $pdo->prepare($sql);        
        $stmt->bindParam(':checkmanid',$S1man,PDO::PARAM_STR);
        $stmt->bindParam(':ID',$dids1,PDO::PARAM_INT);
        $stmt->execute();
        }
    }
    
    if (isset($S2check)and $S2check!='') {
        $S2man=$_POST['S2manager'];//第一季審核者
        $checknum=$checknum+1;
        for ($i=0; $i < $num; $i++) {
            ...
            $sql="UPDATE FA.MMT_FtableD SET checkmanid=:checkmanid WHERE id=:ID";
            $stmt = $pdo->prepare($sql);        
            $stmt->bindParam(':checkmanid',$S2man,PDO::PARAM_STR);
            $stmt->bindParam(':ID',$dids2,PDO::PARAM_INT);
            $stmt->execute();
        }
    }
    
    if (isset($S3check)and $S3check!='') {
        $S3man=$_POST['S3manager'];//第一季審核者
        $checknum=$checknum+1;
        for ($i=0; $i < $num; $i++) {
            ...
            $sql="UPDATE FA.MMT_FtableD SET checkmanid=:checkmanid WHERE id=:ID";
            $stmt = $pdo->prepare($sql);        
            $stmt->bindParam(':checkmanid',$S3man,PDO::PARAM_STR);
            $stmt->bindParam(':ID',$dids3,PDO::PARAM_INT);
            $stmt->execute();
        }
    }
    
    if (isset($S4check)and $S4check!='') {
        $S4man=$_POST['S4manager'];//第一季審核者
        $checknum=$checknum+1;
        for ($i=0; $i < $num; $i++) {
            ...
            $sql="UPDATE FA.MMT_FtableD SET checkmanid=:checkmanid WHERE id=:ID";
            $stmt = $pdo->prepare($sql);        
            $stmt->bindParam(':checkmanid',$S4man,PDO::PARAM_STR);
    ...
    switch ($checknum) {
        case 4:            
            $MasterStr="UPDATE FA.MMT_FtableM SET staus='F' WHERE id=:mid";
            $stmtM = $pdo->prepare($MasterStr);
            $stmtM->bindParam(':mid',$mid,PDO::PARAM_STR);
            $stmtM->execute();
            break;
        case ($checknum>0):
            $MasterStr="UPDATE FA.MMT_FtableM SET status='M' WHERE id=:mid";
            $stmtM = $pdo->prepare($MasterStr);
            $stmtM->bindParam(':mid',$mid,PDO::PARAM_STR);
            $stmtM->execute();
            break;
        ...
        <table class="table my-5">
            ...
            <tr>
                ...
                if(!isset($reportS1[0]['rDate']) or $reportS1[0]['rDate']==''){//第一季沒有保養                   
                    ...
                    echo '<td>';
                    echo $username.'&nbsp&nbsp&nbsp<input type="checkbox" name="S1Check" value="done" >審核';
                    echo '</td>';
                    $checkmS1=$userID;
                    echo "<input type='hidden' name='S1manager' value=\"".$checkmS1."\">";
                }else{//第一季完成審核
                ...
                if(!isset($reportS2[0]['rDate']) or $reportS2[0]['rDate']==''){//第2季沒有保養                   
                    ...
                    echo '<td>';
                    echo $username.'&nbsp&nbsp&nbsp<input type="checkbox" name="S2Check" value="done" >審核';
                    echo '</td>';
                    $checkmS2=$userID;
                    echo "<input type='hidden' name='S2manager' value=\"".$checkmS2."\">";
                }else{//第2季完成審核
                ...
                if(!isset($reportS3[0]['rDate']) or $reportS3[0]['rDate']==''){//第3季沒有保養                   
                    ...
                    echo '<td>';
                    echo $username.'&nbsp&nbsp&nbsp<input type="checkbox" name="S3Check" value="done" >審核';
                    echo '</td>';
                    $checkmS3=$userID;
                    echo "<input type='hidden' name='S3manager' value=\"".$checkmS3."\">";
                }else{//第3季完成審核
                ...
                if(!isset($reportS4[0]['rDate']) or $reportS4[0]['rDate']==''){//第4季沒有保養                   
                    ...
                    echo '<td>';
                    echo $username.'&nbsp&nbsp&nbsp<input type="checkbox" name="S4Check" value="done" >審核';
                    echo '</td>';
                    $checkmS4=$userID;
                    echo "<input type='hidden' name='S4manager' value=\"".$checkmS4."\">";
                }else{//第4季完成審核
