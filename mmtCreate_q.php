<?php
    ...
    switch ($mmtbuildNo) {
        ...
        case 'A':
            ...
            break;
        case 'B':
            if ($mmtequipNo=='B-01-CWTYP') {
                $tid=95;
            }
            if ($mmtequipNo=='B-01-FPYP') {
                $tid=96;
            }
            if ($mmtequipNo=='B-01-ROYP') {
                $tid=97;
            }
            break;
        case 'C':
    ...
    // $reportName=item("SELECT k.id,k.tableName,k.dateKind FROM FA.MMT_KIND AS k LEFT JOIN FA.MMT_equipNo AS n ON k.id=n.tid WHERE n.eid='$mmtequipNo' AND n.fid='$mmtfloorNo' AND n.bid='$mmtbuildNo' AND n.sid='$mmtsysNo' ");
    ...
    $macnum='W'.'-'.$mmtequipNo;//設備編號
    $mmt_a_m_id=$macnum.'-'.$idnum;//主表id
    //撈取檢查項目
    //  $str="SELECT a.id,a.checkName,a.ref,a.checkKind,a.tid FROM FA.MMT_A AS a LEFT JOIN FA.MMT_KIND as k ON a.tid=k.id  WHERE k.sid='Q' AND n.bid='$mmtbuildNo' AND n.fid='$mmtfloorNo' AND n.eid='$mmtequipNo' AND n.sid='$mmtsysNo'";      
    ...
    if (isset($_POST["action"])&&($_POST["action"]=="add")) {
        ...
        for ($i=0; $i < $num; $i++) {                                    
            ...
            $reportCheck="SELECT COUNT(id) FROM FA.MMT_QtableM WHERE id='$mmt_a_m_id' ";
            ...
            if ($reportCheck_query ==0) {                    
                $insert_master_str="INSERT INTO FA.MMT_QtableM(id,bid,fid,eid,rdate,datekind,tid,macNo,remark,emp,status) VALUES ('$MMT_AtableMid','$bid','$fid','$eid','$rTime','$rdatekind','$tid','$macNo','$rmark','$remp','W') ";
                $insert_master =$pdo->exec($insert_master_str);                    
                
                $insert_detail_str="INSERT INTO FA.MMT_QtableD(checkid,ans,mid) VALUES ($check_id,'$ans','$MMT_AtableMid')";
...
<body class="table_bg">
    ...
    <form action="" method="post" name="mmt_table_a">
        ...
        <div class="row my-3">
            ...
            <div class="col text-right">
            <?php
                $datekind=$reportName[0]['dateKind'] ;
