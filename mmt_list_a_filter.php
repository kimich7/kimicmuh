...
<!-- header網頁標題 -->
    ...
    <h1 class="text-center">空調系統</h1>
        <div class="d-flex ">        
            <a href='mmt_list_a.php' type="button" id="backlistmmt" class="btn btn-primary mt-4 rounded d-block mr-3">清除條件</a>
        </div> 
        ...
        <?php
            ...
            switch ($x) {
                case 1://只有keyword
                    ...
                    //總資料數量
                    $totalstr_num="SELECT COUNT(DISTINCT(a.id)) FROM FA.MMT_AtableM as a LEFT JOIN FA.Employee as b on a.emp=b.e_number or a.sremp=b.e_number or a.cemp=b.e_number left join FA.MMT_build as c on a.bid=c.id left join FA.MMT_equip as s on a.eid=s.id left join FA.MMT_KIND as k on a.tid=k.id LEFT JOIN FA.CheckStatus as p ON a.status=p.id WHERE b.cname like '%$keyword%' or c.bName like '%$keyword%' or s.eName like '%$keyword%' or k.tableName like '%$keyword%' or p.checkName like '%$keyword%' or a.remark like '%$keyword%' or a.rdate like '%$keyword%' ";
                    break;
                case 2://只有日期區間
                    //所有的資料 (check_manager IS NULL or check_manager=0) and
        ...
        <?php
            ...
            echo '<table border="0" align="center">';
                echo '<tr>';
                    if ($page_num>1) {
                        @$prev=$page_num-1;
                        echo "<td><a href='mmt_list_a_filter.php?page=1\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>"."[第一頁]".'</a></td>';
                        echo "<td><a href='mmt_list_a_filter.php?page={$prev}\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>"."[<<<上一頁]".'</a></td>';
                        
                    }
                    if ($page_num<$total_page) {
                        @$next=$page_num+1;
                        echo "<td>"."<a href='mmt_list_a_filter.php?page={$next}\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>"."[下一頁>>>]".'</a></td>';
                        echo "<td><a href='mmt_list_a_filter.php?page=$total_page\"&startdate=\"".$start_date."\"&enddate=\"".$end_date."\"&keyword=\"".$keyword."\"&key=next_page'>".'[最末頁]'.'</a></td>';
                    }
                echo '</tr>';
            echo '</table>';
            
            //分頁按鈕一次七頁
            $phpfile = 'mmt_list_a_filter.php';
    ...
    <form action="mmt_list_a_filter.php" method="post" name="mtlist"> 
            <div>
                <div align="left">
                    日期區間搜索：</br>
