        ...
        <div class="list-group mx-5 my-5">            
            ...
            switch ($x) {
                case 1://只有keyword
                    ...
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT a.* FROM FA.Water_System_Record_Master as a LEFT JOIN FA.Employee as b on a.r_member=b.e_number or a.managerID=b.e_number left join FA.Building as c on a.b_number=c.b_number left join FA.Equipment_System_Group as s on a.sysID=s.sysID LEFT JOIN FA.CheckStatus as p ON a.status=p.id WHERE b.cname like '%$keyword%' or c.B_name like '%$keyword%' or s.sysName like '%$keyword%'  or a.rDate like '%$keyword%' or p.checkName like '%$keyword%' ORDER BY recordID DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                ...
                case 3://只有開始時間
                    ...
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT a.* FROM FA.Water_System_Record_Master as a LEFT JOIN FA.Employee as b on a.r_member=b.e_number or a.managerID=b.e_number left join FA.Building as c on a.b_number=c.b_number left join FA.Equipment_System_Group as s on a.sysID=s.sysID WHERE rDate ='$start_date' ORDER BY recordID DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                ...
                case 5://開始+keyword
                    ...
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT a.* FROM FA.Water_System_Record_Master as a LEFT JOIN FA.Employee as b on a.r_member=b.e_number or a.managerID=b.e_number left join FA.Building as c on a.b_number=c.b_number left join FA.Equipment_System_Group as s on a.sysID=s.sysID LEFT JOIN FA.CheckStatus as p ON a.status=p.id WHERE rDate ='$start_date' and (b.cname like '%$keyword%' or c.B_name like '%$keyword%' or s.sysName like '%$keyword%' or p.checkName like '%$keyword%') ORDER BY recordID DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                ...
                case 6://結束+keyword
                    ...
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT a.* FROM FA.Water_System_Record_Master as a LEFT JOIN FA.Employee as b on a.r_member=b.e_number or a.managerID=b.e_number left join FA.Building as c on a.b_number=c.b_number left join FA.Equipment_System_Group as s on a.sysID=s.sysID LEFT JOIN FA.CheckStatus as p ON a.status=p.id WHERE rDate ='$end_date' and (b.cname like '%$keyword%' or c.B_name like '%$keyword%' or s.sysName like '%$keyword%' or p.checkName like '%$keyword%') ORDER BY recordID DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
                ...
                case 7://結束+keyword
                    ...
                    //篩選後給每頁的筆數
                    $sqlstr_page="SELECT a.* FROM FA.Water_System_Record_Master as a LEFT JOIN FA.Employee as b on a.r_member=b.e_number or a.managerID=b.e_number left join FA.Building as c on a.b_number=c.b_number left join FA.Equipment_System_Group as s on a.sysID=s.sysID LEFT JOIN FA.CheckStatus as p ON a.status=p.id WHERE (rDate between '$start_date' and '$end_date')and (b.cname like '%$keyword%' or c.B_name like '%$keyword%' or s.sysName like '%$keyword%' or p.checkName like '%$keyword%') ORDER BY recordID DESC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
                    //總資料數量
            ...
            while ($data_page = $sql_page->fetch()) {
                ...
                $sysName=sql_database('sysName','FA.Equipment_System_Group','sysID',$data_page['sysID']);
                    ...
                    if ($data_page['status']=='F') {
                        ...
                        $status='進行中';                    
                    } elseif($data_page['status']=='W' OR empty($data_page['status'])) {
                        $status='未審核';
                    }
