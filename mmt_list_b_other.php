...
//所有的資料
$ammtmstr="SELECT fileNo,sdid,fileDownloadUserID,checkempId,fileDownloadDate,fileInsertDate,checkdate,eqlocation,fileCycle,filePath,remark,status FROM FA.MMA_getFile WHERE sdid=1";//全部資料
//總資料數量
$ammtmnumstr="SELECT Count(fileNo)FROM FA.MMA_getFile WHERE sdid=1 ";
$ammtmnum=Current($pdo->query($ammtmnumstr)->fetch());//全部數量
$total_num=$ammtmnum;

//----還沒帶入----
//篩選後給每頁的筆數
$sqlstr_page="SELECT fileNo,sdid,fileDownloadUserID,checkempId,fileDownloadDate,fileInsertDate,checkdate,eqlocation,fileCycle,filePath,remark,status FROM FA.MMA_getFile WHERE sdid=1 ORDER BY fileInsertDate ASC OFFSET $startRow_record ROWS FETCH NEXT $pageRow_record ROWS ONLY";
